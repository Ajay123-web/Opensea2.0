...
const GeneralDetails = ({ selectedNft, likes }) => {
  return (
    <div className={style.wrapper}>
      <div className={style.infoContainer}>
        <div className={style.accent}>{selectedNft?.Name}</div>
        <div className={style.nftTitle}>{selectedNft?.Symbol}</div>
        <div className="text-[#2081e2]">{selectedNft.Description}</div>
        <div className={style.otherInfo}>
          <div className={style.ownedBy}>
            Owned by <span className={style.accent}>e88vault</span>
          </div>
          <div className={style.likes}>
            <AiFillHeart className={style.likeIcon} /> {likes}K favorites
          </div>
        </div>
      </div>
